...
## Azure Key Vault
The Azure Key Vault is an managed service that can be used to store secrets, encription keys and certificates.
...
* Lab:
  ...
  * Create a new Application Object/"App Registration" or use an already created one 
  ...
  * Create a Simple Secret in the Key-vault
    ...
    * Create a Simple Java Program to get the secret vaule
...
- We will look into an example of having application hostned on a Virtual Machine that is acessing the blob service.
...
* LAB:
  ...
  * GO to the Storage Account (Access Control IAM)
    ...
    * Click Select Members
      * select the Virutl Machine, click select
      * Review and Assign
  * Create a Java Program to teste the use of Managed Identity .credential(new DefaultAzureCredentialBuilder().build())    
...
* After getting the access token we can use it use the blob storage for example
...
* With **User Assigned Managed Identity** you can assign the same identity to a Virtual Machine and an Azure Function for example 
